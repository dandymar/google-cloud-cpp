...
Options DefaultDataOptions(Options opts) {
  ...
  bool safe_to_enable =
      opts.get<bigtable::experimental::EnableGrpcMetricsOption>() &&
      bigtable_internal::GrpcEnableMetricsIsSafe() && !emulator.has_value() &&
      !project_id.empty();
  ...
  if (safe_to_enable) {
    ...
    if (plugin_status.ok()) {
      ...
      GCP_LOG(WARNING) << "Failed to create Bigtable gRPC OpenTelemetry plugin: "
