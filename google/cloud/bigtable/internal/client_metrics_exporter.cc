...
CreateClientMetricsExporter(
  ...
  // We start with a copy of the client options to preserve credentials and universe domain,
  // but we must unset Bigtable-specific endpoints/authorities to allow default Monitoring defaults.
  auto connection_options = options;
  connection_options.unset<EndpointOption>();
  connection_options.unset<AuthorityOption>();

  auto conn = monitoring_v3::MakeMetricServiceConnection(std::move(connection_options));
